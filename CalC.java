...
public class CalC extends Frame implements ActionListener {
	...
	//Method to handle actions generated by each button
		...
		try{
			...
			if(ae.getSource() == plus){
				if(op == '$') {
					...
					int idx = 0;
					if(op == '-'){ idx = handleMinus(txt); }
					else { idx = txt.indexOf(op); }
			...
			if(ae.getSource() == subtract){
				if(txt.equals("0")){
					...
					for(int i=0;i<txt.length();i++){
						if(txt.charAt(i) == '-') occur++;
					}
					int idx = 0;
					if(occur == 0) { idx = txt.indexOf(op); }
					if(occur == 1) {
						idx = txt.indexOf(op);
					}
					else if(occur == 2) {
						if(op != '-'){ idx = txt.indexOf(op); }
						else {
							if(txt.indexOf('-') == 0){ idx = txt.lastIndexOf('-'); }
							else { idx = txt.indexOf('-'); }
						}
					}
					else if(occur == 3) {
						int L = txt.indexOf('-');
	...
	//Method to handle special case for '-' operator
	static int handleMinus(String txt) {
		int occur = 0;
		for(int i=0;i<txt.length();i++){
			if(txt.charAt(i) == '-') occur++;
		}
	...
	//Method to handle binary operations
	...
	static int getFactorial(int n) throws ArithmeticException{
		if(n < 0)
